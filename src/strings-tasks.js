...
function countVowels(str) {
  return str
    .split('')
    .map((char) => ('aeiouyAEIOUY'.includes(char) ? char : ''))
    .join('').length;
}
...
function isPalindrome(str) {
  const shortStr = str.replace(/[ ,?!]/g, '');
  const len = shortStr.length;
  for (let i = 0; i < len / 2; i += 1) {
    if (shortStr[i].toLowerCase() !== shortStr[len - 1 - i].toLowerCase()) {
      return false;
    }
  }
  return true;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      code > ('A'.charCodeAt(0) + 'Z'.charCodeAt(0)) / 2 &&
      code <= 'Z'.charCodeAt(0)
    ) {
